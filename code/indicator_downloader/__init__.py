...
def getLatestData(tickers, API_SECRET, API_KEY, QUANDL_API, GLASSNODE_API_KEY, AWS_ACCESS_KEY,AWS_SECRET_ACCESS,FREQ, fullpath, fullpath2, write=True):
	data = downloadWrapper(tickers, API_SECRET, API_KEY, FREQ, fullpath, write=write)
	data['date'] = pd.to_datetime(data['date'])
	indicators = merge(QUANDL_API, GLASSNODE_API_KEY,AWS_ACCESS_KEY,AWS_SECRET_ACCESS, fullpath2, write)
